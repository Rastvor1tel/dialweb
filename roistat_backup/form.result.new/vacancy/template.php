...
<? if (($_GET["WEB_FORM_ID"] == $arResult["arForm"]["ID"]) && ($_GET["formresult"] == "addok")): ?><p>Ваша заявка
    отправлена.</p><? endif ?>
...
<? foreach ($arResult["QUESTIONS"] as $arQuestion):
    ...
    <label class="feedback-form__label input-label">
        <input type="text" name="<?= $fieldname ?>" value="<?= $arResult["arrVALUES"][$fieldname] ?>"
               class="input-label__input text-input" <? if ($arQuestion["REQUIRED"] == "Y"): ?>required<? endif ?>>
...
<? elseif ($arQuestion['CAPTION'] == 'Вакансия'):?>
    <input type="hidden" name="<?= $fieldname ?>" value="<?= $APPLICATION->GetProperty('vacancy') ?>">
<? endif ?>
<? endforeach ?>
...
<script>
    ...
    $('input[name="form_text_2"], input[name="form_text_6"]').inputmask({
