...
//if(strlen($arResult['TITLE'])>0) $APPLICATION->SetTitle(html_entity_decode($arResult['TITLE']));
if(strlen($arResult['TITLE'])>0) $APPLICATION->SetPageProperty('title',html_entity_decode($arResult['TITLE']));
//if(strlen($arResult['ROBOTS'])>0) $APPLICATION->SetPageProperty('robots',html_entity_decode($arResult['H1']));
...
if($USER->IsAdmin()) {
// для js-файлов
$APPLICATION->AddHeadScript('/bitrix/components/webfly/meta.edit/templates/.default/script_t.js');

// для css-файлов
$APPLICATION->SetAdditionalCSS('/bitrix/components/webfly/meta.edit/templates/.default/style_t.css');
